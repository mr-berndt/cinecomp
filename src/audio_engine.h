...
typedef enum {
    ...
    /* Legacy-Sammelschalter: 0=RMS, 1=Peak, 2=Dual (RMS oben, Peak unten).
     * Bleibt erhalten, damit alte Presets und alte OSC-Sender weiter
     * funktionieren - er SETZT jetzt aber nur noch die beiden Schalter
     * darunter, und beim Lesen wird er aus ihnen abgeleitet. */
    PARAM_DETECTOR_MODE = PARAM__FLOAT_COUNT,   /* int 0=RMS, 1=Peak, 2=Dual */

    /* ----------------------- Peak oder RMS, je Richtung -------------------
     *
     * Was die Seitenkette misst, getrennt fuer die beiden Stufen:
     * PARAM_DET_UP fuer die Aufwaertsstufe, PARAM_DET_DOWN fuer den Ducker.
     *
     * Die beiden Stufen stellen verschiedene Fragen. Die Aufwaertsstufe hebt
     * Leises an und will wissen, wie LAUT es im Mittel ist - dafuer ist RMS
     * gebaut, ein einzelner Knall soll das Anheben nicht abwuergen. Der
     * Ducker soll dagegen genau diesen Knall erwischen, bevor er da ist,
     * und dafuer taugt nur Peak.
     *
     * Das war bisher der Sammelschalter "Dual", aber nur in der zonalen
     * Architektur und nicht frei waehlbar. Jetzt sind es zwei Schalter.
     */
    ...
    /* ANS ENDE, und das ist kein Schoenheitsfehler: das LADSPA-Plugin und
     * cinecomp-control.lua sprechen die Parameter ueber ihre NUMMER an, und
     * ~/.config/mpv/mpv-cinecomp-params enthaelt diese Nummern. Wer hier in
     * der Mitte einfuegt, verschiebt alles Dahinterliegende und richtet
...
typedef struct {
    ...
    float sc_max;                       /* loudest side-chain since the last read */
    float sc_dn_db;                     /* side-chain the downward stage sees */
...
typedef struct {
    ...
    int   detector_mode;    /* Legacy, siehe oben - beim Laden ausgewertet */
    int   det_up;           /* 0=RMS, 1=Peak */
    int   det_down;
    int   downward_en;
