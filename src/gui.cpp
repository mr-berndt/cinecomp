...
namespace clr {
    ...
    // The duck red is a curve colour and deliberately soft. The peak hold
    // has to be seen at a glance from the sofa, so it gets a real red.
    constexpr ImVec4 hold_red    = {1.000f, 0.130f, 0.130f, 1.0f};
}
...
/* Peak hold for the curve marker. It jumps up at once, stands for three
 * seconds and then falls back - the same law the bar holds use, so both
 * halves of the window behave alike. Fed from the engine's sc_max, not from
 * the value that happened to be current when the frame was drawn: at twenty
 * frames a second the current value misses most of the sound. */
static float g_sc_hold     = -120.0f;
static float g_sc_hold_age = 0.0f;

static void sc_hold_update(float sc_max, float dt) {
    const float sticky = 3.0f;      /* seconds it stands still */
    const float dbps   = 12.0f;     /* then falls this fast */
    if (!std::isfinite(sc_max)) return;
    if (sc_max > g_sc_hold) {
        g_sc_hold     = sc_max;
        g_sc_hold_age = 0.0f;
    } else {
        g_sc_hold_age += dt;
        if (g_sc_hold_age > sticky)
            g_sc_hold = std::max(-120.0f, g_sc_hold - dbps * dt);
    }
}
...
static void draw_curve(ImVec2 area_min, ImVec2 area_max,
    ...
    // Peak hold - a thin vertical tick crossing the curve at the loudest
    // recent level, sticking out a little above and below. Its height is the
    // curve's own value there, not the gain applied right now: the loud
    // moment is over, only what it would get is still true.
    if (std::isfinite(g_sc_hold) && g_sc_hold > x_lo) {
        float hx = std::min(std::max(g_sc_hold, x_lo), x_hi);
        float gx = x2px(hx);
        GainPair gp = gain_target_view(hx, arch_mode);
        float gy = gp.up;
        if (gy < y_lo) gy = y_lo;
        if (gy > y_hi) gy = y_hi;
        float cy = y2px(gy);
        const float half = 8.0f;
        dl->AddLine(ImVec2(gx, cy - half), ImVec2(gx, cy + half),
                    col32(clr::hold_red), 1.5f);
    }
...
int main(int argc, char **argv) {
    ...
    // Architecture is fixed to zonal v2 (Classic/v1 retired).
    engine_set_param_i(PARAM_ARCHITECTURE_MODE, 2);
    // Die alte Klemme auf "Dual ist tot" entfaellt seit dem 21.9.2026: die
    // Kombination RMS oben / Peak unten ist jetzt ganz normal einstellbar,
    // ein altes state.ini mit detector_mode = 2 bekommt also genau das,
    // was dort stand, statt auf Peak/Peak gezwungen zu werden.
    ...
    /* Ten a second was enough for numbers, but not for a peak meter: a bar
     * that is sampled ten times a second cannot look alive, however good the
     * data behind it is. Twenty, on Nicola's word (13.9.2026). */
    const double IDLE_DT = 1.0 / 20.0;
    ...
    while (!glfwWindowShouldClose(win) && !g_quit) {
        ...
        // ===== Header =================================================
        {
            ...
            // bypass remain here.
            ImGui::Spacing();
            // Zwei Schalter statt einem: die Aufwaertsstufe und der Ducker
            // stellen verschiedene Fragen. Oben geht es darum, wie LAUT es im
            // Mittel ist - dafuer ist RMS gebaut, ein einzelner Knall soll das
            // Anheben nicht abwuergen. Der Ducker soll genau diesen Knall
            // erwischen, und dafuer taugt nur Peak.
            const char *det_names[2] = { "RMS", "Peak" };
            int det_u = engine_get_param_i(PARAM_DET_UP);
            int det_d = engine_get_param_i(PARAM_DET_DOWN);
            ImGui::TextColored(clr::text_sand, "Det auf"); ImGui::SameLine();
            for (int i = 0; i < 2; i++) {
                if (i > 0) ImGui::SameLine();
                if (seg_button(det_names[i], det_u == i, ImVec2(46, 24),
                               clr::mint)) {
                    engine_set_param_i(PARAM_DET_UP, i);
                    mark_dirty();
                }
            }
            ImGui::SameLine(0, 10);
            ImGui::TextColored(clr::text_sand, "ab"); ImGui::SameLine();
            for (int i = 0; i < 2; i++) {
                if (i > 0) ImGui::SameLine();
                // Eigene Kennung, sonst haelt ImGui die vier Knoepfe fuer
                // zwei - die Beschriftung ist die Kennung.
                char lbl[16]; std::snprintf(lbl, sizeof lbl, "%s##ab", det_names[i]);
                if (seg_button(lbl, det_d == i, ImVec2(46, 24), clr::mint)) {
                    engine_set_param_i(PARAM_DET_DOWN, i);
                    mark_dirty();
                }
            }
            ImGui::SameLine(0, 10);
        ...
        ImGui::BeginChild("##meters", ImVec2(right_w, top_h), true);
        {
            ...
            /* The hold in red, right beside the live value: the loudest
             * moment of the last three seconds - the same number the red
             * tick on the curve is standing on. */
            if (std::isfinite(g_sc_hold) && g_sc_hold > -60.0f) {
                float vw = ImGui::CalcTextSize(buf).x;
                char hbuf[16];
                std::snprintf(hbuf, sizeof(hbuf), "%.1f", g_sc_hold);
                dl->AddText(ImVec2(text_x + vw + 8, y), col32(clr::hold_red), hbuf);
            }
            y += 18;
